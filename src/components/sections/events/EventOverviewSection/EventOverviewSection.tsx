    ...
    <div className={className}>
      ...
      {overview.items.length > 0 && (
        <div className="mt-10 w-full overflow-x-auto">
          <div className="flex divide-x divide-black/10 border-y border-black/10">
            {overview.items.map((item) => (
              <div key={item.title} className="flex-1 py-6 pr-6 first:pl-0 sm:pr-10 [&:not(:first-child)]:pl-6 sm:[&:not(:first-child)]:pl-10">
                <p className="font-heading font-normal leading-none text-mint" style={{ fontSize: "clamp(2.5rem, 6vw, 5.625rem)" }}>{item.count}</p>
